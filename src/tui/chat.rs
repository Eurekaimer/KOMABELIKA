...
pub struct ChatView<'a> {
    ...
    pub input: &'a str,
    pub input_title: &'a str,
    pub credential_entry: bool,
    pub message_queued: bool,
    pub bold_text: bool,
...
pub fn render(frame: &mut Frame<'_>, view: ChatView<'_>) {
    let area = frame.area();
    let border_type = border_type(view.border_style);
    ...
    let input = Paragraph::new(view.input)
        .style(text_style(view.bold_text))
        .block(
            Block::default()
                .title(view.input_title)
                .borders(Borders::ALL)
                .border_type(border_type)
    ...
    let footer_text = view.error.map_or_else(
        || {
            if view.credential_entry {
                "输入 API Key · Enter 验证并保存 · Esc 取消 · 内容不会显示".to_owned()
            } else if view.message_queued {
                "消息已排队，将在当前回复完成后发送".to_owned()
            } else {
                format!(
