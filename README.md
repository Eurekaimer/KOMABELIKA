...
输入 `/` 会自动显示命令候选；继续输入 `/c` 会收窄到 `/clear`。用 `↑`、`↓` 选择、`Tab` 补全；`Tab` 只负责补全，不再切换区域。模型选择器使用 `↑`、`↓` 移动，`Enter` 确认，`Esc` 关闭。

## 键位

- 输入为空时按 `t`：在下方输入框和上方历史区之间切换；输入已有草稿时可用 `Ctrl+T`
- 历史区 `k` / `↑`：向上滚动，`j` / `↓`：向下滚动
- 历史区 `PageUp` / `PageDown`：整段翻页
- 历史区 `g` / `Home`：跳到开头，`G` / `End`：回到底部
- 历史区 `t` 或 `Esc`：返回输入框
- `Enter`：发送
- `Shift+Enter`：换行
- `Esc`：取消当前生成
